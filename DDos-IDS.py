...
def preprocess_flows(df):
    ...
    # More accurate approximations for missing CIC-DDoS features based on NFStream outputs
    # TCP Window sizes (often huge indicators in DDoS models, especially SYN floods)
    df['Init_Win_bytes_forward'] = df['Total Fwd Packets'] * 29200 # Arbitrary common window size approximation if missing
    df['Init_Win_bytes_backward'] = df['Total Backward Packets'] * 29200
    df['act_data_pkt_fwd'] = df['Total Fwd Packets']
    df['min_seg_size_forward'] = 20 # standard IPv4 header min

    # Flow Idle and Active Times
    ...
    df['Active Max'] = 0
    df['Active Min'] = 0
    df['Inbound'] = 1 # Assume inbound for simple IDS placement
...
def detect(df, raw_df, cycle):
    ...
    # User requested to NOT rely on volumetric threshold rules, letting ML models do the heavy lifting
